...
if __name__ == "__main__":
	...
	os.chdir("train/")
	for i in tqdm(label_to_urls.keys()):
		os.mkdir(str(i))
		os.chdir(str(i))
		k = 0
		for j in tqdm(label_to_urls[i][:round(train*(len(label_to_urls[i])))]):
	...
	for i in tqdm(label_to_urls.keys()):
		...
		os.chdir(str(i))
		k = 0
		for j in tqdm(label_to_urls[i][round(0.8*(len(label_to_urls[i]))):]):
			downloader(j , str(i)+str(k))
			k+=1
		os.chdir("../")
